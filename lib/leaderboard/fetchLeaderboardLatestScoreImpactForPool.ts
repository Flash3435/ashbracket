...
export type LeaderboardLatestScoreImpactResult = {
...
async function computeBracketImpactFallback(
  ...
  const matchResults: ScoreImpactMatchResult[] = matchCodes.map(
    (matchCode): ScoreImpactMatchResult => ({
      matchCode,
...
function parseBracketImpactFromMetadata(
  metadata: Record<string, unknown>,
  displayNameByParticipantId: ReadonlyMap<string, string>,
): LeaderboardBracketImpactResult | null {
  const parsedRows = parseBracketImpactParticipantRows(metadata);
  if (parsedRows.size === 0) return null;
...
/**
 * Loads momentum, bracket impact, and latest-event context from one score-impact row.
 */
export async function fetchLeaderboardLatestScoreImpactForPool(
  supabase: SupabaseClient,
  poolId: string,
  ...
  let momentum: LeaderboardMomentumResult | null = null;
  if (validation.valid) {
    const previousRows = parsePreviousStandingsFromMetadata(metadata);
    if (previousRows) {
      momentum = buildLeaderboardMomentum({ currentRows, previousRows });
    }
    ...
    momentum = { hasPreviousSnapshot: false, rows: [] };
  }

  const hasValidSnapshot = momentum?.hasPreviousSnapshot === true;
  const event = hasValidSnapshot
    ? parseLatestScoreEventContext(metadata, { hasValidSnapshot: true })
    : null;

  let bracketImpact = parseBracketImpactFromMetadata(metadata, displayNameByParticipantId);
  if (!bracketImpact && hasValidSnapshot) {
    bracketImpact = await computeBracketImpactFallback(
      supabase,
      poolId,
      metadata,
