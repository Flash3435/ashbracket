-- World Cup 2026 live pools: backfill missing/incorrect pick lock deadlines and
-- default new pool creation to June 10, 2026 11:59 p.m. Eastern Time
-- (2026-06-11 03:59:00+00).

-- ---------------------------------------------------------------------------
-- Backfill existing live fifa_wc_2026 pools
-- ---------------------------------------------------------------------------

UPDATE public.pools p
SET
  lock_at = '2026-06-11 03:59:00+00'::timestamptz,
  updated_at = now()
...
WHERE p.tournament_edition_id = te.id
  ...
  AND (
    p.lock_at IS NULL
    OR p.lock_at = '2026-06-08 17:59:00+00'::timestamptz
    OR p.lock_at = '2026-06-11 05:59:00+00'::timestamptz
  );

-- ---------------------------------------------------------------------------
-- create_pool_for_current_user: default lock_at for new live WC 2026 pools
-- ---------------------------------------------------------------------------
...
DECLARE
  ...
  v_n int := 0;
  v_official_id uuid;
  v_wc_lock timestamptz := '2026-06-11 03:59:00+00'::timestamptz;
BEGIN
...
-- ---------------------------------------------------------------------------
-- create_pool_with_owner: default lock_at for new live WC 2026 pools only
-- ---------------------------------------------------------------------------
