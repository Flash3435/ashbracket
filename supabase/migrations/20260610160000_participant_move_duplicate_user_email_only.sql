-- Participant move duplicate protection: match linked account or email only.
-- Display-name-only collisions should not block moves between pools.
...
COMMENT ON FUNCTION public.move_world_cup_participant_to_pool(uuid, uuid, uuid) IS
  'Moves a World Cup participant between compatible pools. Duplicate protection uses linked user_id or email only.';
