-- Admin: move a World Cup participant (and picks) between compatible pools in one transaction.
...
BEGIN
  ...
  IF NOT public.ashbracket_can_manage_pool(p_source_pool_id) THEN
    RAISE EXCEPTION 'not authorized for source pool' USING ERRCODE = '42501';
  END IF;

  IF NOT public.ashbracket_can_manage_pool(p_destination_pool_id) THEN
...
COMMENT ON FUNCTION public.move_world_cup_participant_to_pool(uuid, uuid, uuid) IS
  'Moves a World Cup participant row, predictions, and pool activity from one compatible pool to another. Caller must manage both pools.';
